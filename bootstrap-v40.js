(() => {
  ...
  function baseUrl() {
    const script = [...document.querySelectorAll('script[src]')]
      .find(el => String(el.src).includes('/answer-me-/bootstrap-v40.js'));
    return script?.src
      ? new URL('.', script.src)
  ...
  async function boot() {
    // Must load before race-core so CHAT_COMPLETION_SETTINGS_READY is captured
    // before side candidates are dispatched from the core listener.
    ...
    sync();
    console.log(`[💢 Answer Me] wrapper ${VERSION} ready · side requests inherit main generation recipe`);
  }
