(() => {
    ...
    function baseUrl() {
        const script = [...document.querySelectorAll('script[src]')]
            .find(el => String(el.src).includes('/answer-me-/bootstrap-v12.js'));
    ...
    async function boot() {
        // 先包 transport，再让质量守卫包在外层：假流整包也必须通过空回验尸。
        await inject('model-router-v12.js', 'model-router-v12');
        ...
        console.log(`[💢 Answer Me] wrapper ${VERSION} ready · model groups + global stream mode`);
    }
