(() => {
    ...
    function baseUrl() {
        const script = [...document.querySelectorAll('script[src]')]
            .find(el => String(el.src).includes('/answer-me-/bootstrap-v13.js'));
    ...
    async function boot() {
        // transport 先包底层；quality guard 后包外层。假流整包也必须经过空回验尸。
        ...
        console.log(`[💢 Answer Me] wrapper ${VERSION} ready · per-site model catalogs + global stream mode`);
    }
