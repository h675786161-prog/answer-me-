(() => {
  ...
  const FLAG = '__answerMeDiagnosticsV35';
  const WRAP_FLAG = '__answerMeDiagnosticsWrappedV35';
  ...
  function classifyAbort(reason) {
    const s = String(reason || '').toLowerCase();
    if (!s) return 'aborted';
    if (s.includes('winner-chosen') || s.includes('winner chosen')) return 'aborted_after_winner';
    if (s.includes('stream start timeout')) return 'stream_start_timeout';
    if (s.includes('cold-timeout') || s.includes('cold timeout')) return 'cold_timeout';
    if (s.includes('round-cancelled') || s.includes('round cancelled')) return 'round_cancelled';
    if (s.includes('started stream stalled')) return 'started_stream_stalled';
    return 'aborted';
  }
  ...
  function wrapService() {
    ...
    service.sendRequest = async function(profileId, prompt, maxTokens, custom = {}, overridePayload = {}) {
      ...
      try {
        ...
        const aborted = !!custom?.signal?.aborted;
        const abortReason = aborted ? String(custom.signal.reason || '') : '';
        push(aborted ? 'request_aborted' : 'request_error', {
          ...meta,
          ms: Math.round(perf() - started),
          category: aborted ? classifyAbort(abortReason) : 'provider_or_transport_error',
          abortReason,
      ...
      return function answerMeDiagnosticFactory() {
        ...
        return (async function*() {
          ...
          try {
            ...
            const aborted = !!custom?.signal?.aborted;
            const abortReason = aborted ? String(custom.signal.reason || '') : '';
            push(aborted ? 'stream_aborted' : 'stream_error', {
              ...meta,
              ms: Math.round(perf() - started),
              chunks,
  ...
  function snapshot() {
    ...
    return {
      ...
      retry: window.AnswerMe?.retry || null,
      activeRound: round ? {
        id: round.id,
        retryNo: round.retryNo,
        startedAt: round.startedAt,
        singleApi: round.candidates?.size === 1,
  ...
  function text() {
    ...
    return [
      ...
      '',
      '--- 重试状态 ---',
      JSON.stringify(snap.retry, null, 2),
      '',
      '--- 当前赛马状态 ---',
  ...
  function mountControls() {
    const host = document.querySelector('#answer_me_compact_ui_v23 .advanced')
      || document.querySelector('#answer_me_settings .answer-me-grid');
    if (!host) return false;
    if (!document.querySelector('#answer_me_copy_diag_v35')) {
      const copyBtn = document.createElement('button');
      copyBtn.id = 'answer_me_copy_diag_v35';
      copyBtn.type = 'button';
    ...
    if (!document.querySelector('#answer_me_clear_diag_v35')) {
      const clearBtn = document.createElement('button');
      clearBtn.id = 'answer_me_clear_diag_v35';
  ...
  (function boot() {
    ...
    if (wrapped && mounted) {
      console.log(`[💢 Answer Me] diagnostics ${VERSION} ready · abort-aware race-only trace`);
      return;
    }
