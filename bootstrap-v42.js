(() => {
  ...
  function baseUrl() {
    const script = [...document.querySelectorAll('script[src]')]
      .find(el => String(el.src).includes('/answer-me-/bootstrap-v42.js'));
    return script?.src
      ? new URL('.', script.src)
  ...
  async function boot() {
    ...
    // Snapshot native ST connection before keyword-router v18 does its legacy
    // reconcile. v42 restores it and turns selected side models into per-request
    // overrides instead of persistent Connection Profile mutations.
    ...
    sync();
    console.log(`[💢 Answer Me] wrapper ${VERSION} ready · race no longer hijacks native ST connection`);
  }
