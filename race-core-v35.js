(() => {
  ...
  const EXT = 'answerMe';
  const VERSION = '0.5.1-beta.35';
  const DISPLAY = '💢 Answer Me';
  const RETRY_DELAYS = [2000, 5000, 12000, 25000];
  const STREAM_START_MIN_MS = 30000;
  const STREAM_START_MAX_MS = 45000;
  ...
  function streamStartWaitMs() {
    const cold = Number(settings?.coldTimeoutMs || 90000);
    return Math.max(10000, Math.min(STREAM_START_MAX_MS, Math.max(STREAM_START_MIN_MS, cold - 5000)));
  }
  ...
  function isSingleApiRound(round) {
    return !!round && round.candidates?.size === 1;
  }
  ...
  function currentIsSelected() {
    return (settings?.profileIds || []).map(String).includes(currentProfileId());
  }
  ...
  function statusIcon(c) {
    ...
    if (c.error) return '❌';
    if (c.started) return '🟢';
    if (c.fallbackUsed) return '🟠';
    return '⚫';
  }

  function statusText(c) {
    if (c.winner) return '抢答成功';
    if (c.finished) return c.isOriginal ? '已完成' : (c.fallbackUsed ? '整包抢救成功 · Swipe' : '已完成 · Swipe');
    if (c.aborted) return c.error || '已断开';
    if (c.error) return c.error;
    if (c.started) return '已经开口，准许继续说';
    if (c.fallbackUsed) return c.fallbackReason === 'stream-start-timeout'
      ? '首流等待超时 · 已切整包抢救'
      : '真流请求失败 · 已切整包抢救';
    return `等待真流开口 · 最多 ${Math.round(streamStartWaitMs() / 1000)}s`;
  }
  ...
  function renderFloatingStatus() {
    ...
    if (!activeRound) {
      if (retryState.timer) {
        ...
        const mode = retryState.singleApi ? '单 API 追杀' : '全军覆没追杀';
        meta.textContent = `☠️ ${mode} · ${left}s 后第 ${retryState.count + 1} 次`;
    ...
    const singleTag = isSingleApiRound(round) ? ' · 单 API' : '';
    meta.textContent = `第 ${round.id} 轮${singleTag}${retryTag}`;
  ...
  function abortCandidate(c, reason = '已断开', kind = 'aborted') {
    if (!c || terminal(c)) return;
    c.aborted = true;
    c.abortKind = kind;
    c.error = reason;
    c.finishedAt = now();
    try { c.attemptController?.abort?.(`Answer Me: ${kind}`); } catch {}
    try { c.controller?.abort?.(`Answer Me: ${kind}`); } catch {}
    scheduleRender();
  }
  ...
  function abortRound(reason = 'cancelled', { stopOriginal = false, clearRetry = false } = {}) {
    ...
    for (const c of round.candidates.values()) {
      if (!c.isOriginal && !terminal(c)) abortCandidate(c, reason, 'round-cancelled');
    }
    const original = round.candidates.get('__original__');
    if (original && !terminal(original)) {
      original.aborted = true;
      original.abortKind = 'round-cancelled';
  ...
  async function installSideWinnerImmediately(round, c) {
    ...
    if (original && !terminal(original)) {
      original.aborted = true;
      original.abortKind = 'winner-chosen';
  ...
  function killColdAfterWinner(round) {
    ...
    for (const c of round.candidates.values()) {
      ...
      if (c.isOriginal) {
        ...
        abortCandidate(c, '零正文 token · 赢家已出，已断开', 'winner-chosen');
      }
    }
  }
  ...
  async function requestNonStream(round, c, prompt, maxTokens, reason = '') {
    ...
    c.fallbackUsed = true;
    if (!c.fallbackReason) c.fallbackReason = reason || 'nonstream';
    scheduleRender();
  ...
  async function requestStreamWithFallback(round, c, prompt, maxTokens) {
    ...
    const startWait = streamStartWaitMs();
    activityTimer = setTimeout(() => {
      if (round !== activeRound || c.controller.signal.aborted || sawStreamActivity) return;
      fallbackRequested = true;
      c.fallbackReason = 'stream-start-timeout';
      try { attempt.abort('Answer Me: stream start timeout'); } catch {}
    }, startWait);
    ...
    try {
      ...
      if (validReply(c.text) && stallRequested) return;
      if (validReply(c.text)) throw new Error(`流式中途断开（已收到 ${c.text.length} 字，未作为完整回复结算）`);
      if (!c.fallbackReason) c.fallbackReason = fallbackRequested ? 'stream-start-timeout' : 'provider-stream-error';
      if (!sawText || fallbackRequested || !validReply(c.text)) {
        await requestNonStream(round, c, prompt, maxTokens, c.fallbackReason);
        return;
      }
      throw error;
    }
  ...
  async function runSideCandidate(round, c, prompt, maxTokens) {
    ...
    try {
      if (c.mode === 'fake') {
        c.fallbackReason = 'fake-mode';
        await requestNonStream(round, c, prompt, maxTokens, 'fake-mode');
      } else {
        await requestStreamWithFallback(round, c, prompt, maxTokens);
      }
      if (round !== activeRound || c.controller.signal.aborted) return;
  ...
  function retryEnabledFor(round) {
    return isSingleApiRound(round) ? settings?.singleApiRetryEnabled !== false : !!settings?.autoRetryEnabled;
  }

  function scheduleAutoRetry(reason, singleApi = false) {
    if (!settings?.enabled || retryState.timer) return false;
    if (singleApi ? settings?.singleApiRetryEnabled === false : !settings?.autoRetryEnabled) return false;
    if (retryState.count >= settings.maxRetryRounds) {
      lastResult = `☠️ 连续 ${settings.maxRetryRounds} 次追杀仍失败`;
      toast('error', `已经追着肘了 ${settings.maxRetryRounds} 轮还是失败。`, '💢 Answer Me');
      scheduleRender();
      return false;
    }
    const delay = retryDelayFor(retryState.count);
    retryState.lastReason = reason;
    retryState.singleApi = !!singleApi;
    retryState.dueAt = now() + delay;
    retryState.timer = setTimeout(async () => {
      retryState.timer = null;
      retryState.dueAt = 0;
      const enabled = retryState.singleApi ? settings?.singleApiRetryEnabled !== false : !!settings?.autoRetryEnabled;
      ...
      try { await ctx?.generate?.('regenerate'); }
        ...
        retryState.nextGenerationIsRetry = false;
        scheduleAutoRetry(String(e?.message || e || '重试失败'), retryState.singleApi);
      }
    }, delay);
    startStatusTimer();
    scheduleRender();
    toast('warning', `${singleApi ? '单 API 失败' : '全军覆没'}，${delay / 1000}s 后第 ${retryState.count + 1}/${settings.maxRetryRounds} 次追杀。`, '💢 你他妈倒是回我啊');
    return true;
  }
  ...
  async function maybeHandleTotalFailure(round, reason = '全员失败') {
    ...
    if (round.coldTimer) clearTimeout(round.coldTimer);
    const singleApi = isSingleApiRound(round);
    lastResult = singleApi ? '☠️ 单 API 本轮失败' : '☠️ 本轮全军覆没';
    activeRound = null;
    if (retryEnabledFor(round)) scheduleAutoRetry(reason, singleApi);
    scheduleRender();
  }
    ...
    round.coldTimer = setTimeout(async () => {
      ...
      for (const c of round.candidates.values()) {
        ...
        if (c.isOriginal) {
          ...
          internalStopUntil = now() + 1200;
          try { ctx?.stopGeneration?.(); } catch {}
        } else abortCandidate(c, '冷暴力等待上限 · 零正文', 'cold-timeout');
      }
  ...
  async function startRace(prompt) {
    ...
    if (!sides.length && !currentIsSelected()) return;
    ...
    startStatusTimer();
    scheduleRender();
    log(`第 ${round.id} 轮开赛${isSingleApiRound(round) ? '（单 API 守尸模式）' : ''}`, sides.map(p => `${p.name} · ${p.model}`));
    for (const c of round.candidates.values()) {
  ...
  async function finishOriginalFromMessage() {
    ...
    if (!validReply(text)) {
      original.error = '空回 / 原生请求失败或有效正文不足';
      scheduleRender();
  ...
  function onGenerationStopped() {
    ...
    if (now() < manualStopUntil) {
      abortRound('用户停止生成', { stopOriginal: false, clearRetry: true });
      return;
    }
    const original = round.candidates.get('__original__');
    if (isSingleApiRound(round) && original && !original.started && !round.originalFinished) {
      original.error = '原生请求停止 · 零正文，按失败处理';
      original.finishedAt = now();
      round.originalFinished = true;
      void maybeHandleTotalFailure(round, original.error);
      return;
    }
    abortRound('用户停止生成', { stopOriginal: false, clearRetry: true });
  }
  ...
  function bindManualStopMarker() {
    if (document.documentElement.dataset.answerMeManualStopV35 === '1') return;
    document.documentElement.dataset.answerMeManualStopV35 = '1';
    document.addEventListener('pointerdown', e => {
      const target = e.target?.closest?.('#mes_stop, .mes_stop, [data-answer-me-stop], [title*="Stop"], [title*="停止"]');
      if (target) manualStopUntil = now() + 2500;
    }, true);
  }
  ...
  function mountSettings() {
    ...
    root.innerHTML = `
      ...
      <div class="answer-me-note">第一条完整有效回复当主回复；已吐正文的站继续完成并收进 Swipe。真流首事件不再写死 12 秒，默认会等约 45 秒再切整包；只选一个 API 时也会接管失败/空回并按同一退避链自动重试。</div>
      <div class="answer-me-grid">
        ...
        <label><span>失败后最多再肘几轮</span><input id="answer_me_retry_rounds" type="number" min="0" max="8" step="1"></label>
      ...
      <label class="answer-me-check-row"><input id="answer_me_auto_retry" type="checkbox"><span>多站全军覆没后自动重试整轮赛马</span></label>
      <label class="answer-me-check-row"><input id="answer_me_single_retry" type="checkbox"><span>只选一个 API 时，失败 / 空回也自动重试</span></label>
    ...
    $('#answer_me_single_retry').checked = settings.singleApiRetryEnabled !== false;
    ...
    $('#answer_me_auto_retry').addEventListener('change', e => { settings.autoRetryEnabled = e.currentTarget.checked; if (!settings.autoRetryEnabled && !retryState.singleApi) resetRetryChain('多站自动重试已关闭'); saveSettings(); });
    $('#answer_me_single_retry').addEventListener('change', e => { settings.singleApiRetryEnabled = e.currentTarget.checked; if (!settings.singleApiRetryEnabled && retryState.singleApi) resetRetryChain('单 API 自动重试已关闭'); saveSettings(); });
  ...
  window.AnswerMe = {
    ...
    get retry() { return { count: retryState.count, timer: retryState.timer, dueAt: retryState.dueAt, lastReason: retryState.lastReason, singleApi: retryState.singleApi }; },
    profiles,
  ...
  async function boot() {
    ...
    mountSettings();
    bindEvents();
    bindManualStopMarker();
    log(`${VERSION} ready · stream-start wait=${Math.round(streamStartWaitMs()/1000)}s · single API retry=${settings.singleApiRetryEnabled !== false}`);
  }
