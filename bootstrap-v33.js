(() => {
  ...
  function baseUrl(){const s=[...document.querySelectorAll('script[src]')].find(e=>String(e.src).includes('/answer-me-/bootstrap-v33.js'));return s?.src?new URL('.',s.src):new URL('/scripts/extensions/third-party/answer-me-/',window.location.origin)}
  ...
  async function waitContext(){for(let i=0;i<100&&!window.SillyTavern?.getContext;i++) await sleep(100)}
  async function boot(){
    await waitContext();
    await inject('settlement-guard-v33.js','settlement-guard-v33');
    ...
    console.log(`[💢 Answer Me] wrapper ${VERSION} ready · settlement guard + heartbeat watchdog armed`);
  }
