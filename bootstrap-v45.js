(() => {
  ...
  function baseUrl() {
    const script = [...document.querySelectorAll('script[src]')]
      .find(el => String(el.src).includes('/answer-me-/bootstrap-v45.js'));
    return script?.src
      ? new URL('.', script.src)
  ...
  async function boot() {
    ...
    // v45 replaces the entire legacy model/site routing stack. The retired
    // modules changed profile.model, #connection_profiles and native stream
    // toggles; the new stack only writes Answer Me's own race settings.
    await inject('model-router-v45.js', 'model-router-v45');
    await inject('site-selector-v45.js', 'site-selector-v45');
    ...
    sync();
    console.log(`[💢 Answer Me] wrapper ${VERSION} ready · legacy plug-hijacking router stack retired`);
  }
