(() => {
    ...
    function baseUrl() {
        const script = [...document.querySelectorAll('script[src]')]
            .find(el => String(el.src).includes('/answer-me-/bootstrap-v9.js'));
    ...
    async function boot() {
        ...
        // beta.9 不再加载旧 stall-guard-v7：它会把“正文已写完但 SSE 不关”误判成失败。
        await inject('soft-settle-v9.js', 'soft-settle');
        ...
        console.log(`[💢 Answer Me] wrapper ${VERSION} ready · soft settle + refresh recovery loaded`);
    }
