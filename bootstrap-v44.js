(() => {
  ...
  function baseUrl() {
    const script = [...document.querySelectorAll('script[src]')]
      .find(el => String(el.src).includes('/answer-me-/bootstrap-v44.js'));
    return script?.src
      ? new URL('.', script.src)
  ...
  async function boot() {
    ...
    // v44 deliberately retires exact-model-picker-v24. Manual override UI had
    // grown into a second model manager and also carried a permanent observer +
    // polling loop. Automatic matching is now the only normal path.
    ...
    sync();
    console.log(`[💢 Answer Me] wrapper ${VERSION} ready · manual model overrides retired`);
  }
